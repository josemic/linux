...
Activation / deactivation of Promiscuous mode
---------------------------------------------
...
Set J1939 message priority
--------------------------

.. code-block:: C

    int j1939_prio = 1; // value range: 0..7, default = 6
...
Sending of large J1939 (E)TP messages
-------------------------------------

When sending of (large) j1939 messages, the return value of the send command has to be checked. If the functions returns less bytes than has been tried to send, the remaining bytes have to be send.
Calling send() with the remaining bytes, doesn't start a new message, the data is queued for the current running message transfer.
